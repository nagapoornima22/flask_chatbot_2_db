...
class ChatBot(object):
    ...
    def response(self, sentence, question=None, userID='123', show_details=False):
        ...
        if results:
                        ...
                        if "command" in i:
                            if (i["command"] != None):
                                ...
                                if question != sentence:
                                    ...
                                    return commandClassObj.execute(sentence)
                                else:
                                    res = resQuestion + '!' + 'command'
                                    print(res)
                                    return res

                                # empId = input(random.choice(i['Questions'][0]["pattern"]))
                                # print(empId)
                                # commandToExecute = random.choice(i['command'])
                                # commandClassObj = getModuleClass(commandToExecute)
                                # return commandClassObj.execute(sentence)
            ...
            return "Sorry i didnot understand your question"
