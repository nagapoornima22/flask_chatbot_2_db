...
class ChatBot(object):
    ...
    def response(self, sentence, question=None, userID='123', show_details=False):
        if sentence == question:
            ...
            print("que :",question)
            results = self.classify(question)
        context = {}
                        ...
                        if "command" in i:
                            if (i["command"] != None):
                                ...
                                if question != sentence:
                                    print('commands loop')
                                    print(random.choice(i['command']))
                                    commandToExecute = random.choice(i['command'])
                                    commandClassObj = getModuleClass(commandToExecute)
                                    print(commandClassObj)
                                    return ApiResponse("Array", commandClassObj.execute(sentence)).__dict__
                                else:
                                    resQuestion = random.choice(i['Questions'][0]["pattern"])
                                    print(resQuestion)
                                    return ApiResponse("Question" , resQuestion).__dict__
                        else:
                            return ApiResponse("Text" ,random.choice(i['responses'])).__dict__
        else:
